...
/** Kinh nghiệm — dữ liệu được server đọc sẵn (SSR/ISR), không fetch phía client */
export default function Experience({ items: initial }) {
  const { t, lang } = useLang()
  const items = Array.isArray(initial) && initial.length ? initial : fallbackExperience
        ...
        {items.map((job, i) => {
          ...
          return (
            <Reveal key={job.id ?? job.company} delay={(i % 2) * 100} className="h-full">
              <Link
                href={`/experience/${job.slug || job.id}`}
